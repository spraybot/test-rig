...
//		sbp_register_callback(&state, SBP_VEL_ECEF, &vel_ecefCallback, (void*) this, &vel_ecef_callback_node);
    ...
    if ((sbp_ned.flags&0x7) != 0) {
        ...
        rtk_odom_msg->pose.covariance[21] = 1.0e-3;  // x rotation = 3, 3
        rtk_odom_msg->pose.covariance[28] = 1.0e-3;  // y rotation = 4, 4
        rtk_odom_msg->pose.covariance[35] = 1.0e-3;  // z rotation = 5, 5
        ...
        rtk_odom_msg->twist.covariance[21] = 1.0e-3;  // x rotational velocity = 3, 3
        rtk_odom_msg->twist.covariance[28] = 1.0e-3;  // y rotational velocity = 4, 4
        rtk_odom_msg->twist.covariance[35] = 1.0e-3;  // z rotational velocity = 5, 5
