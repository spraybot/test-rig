...
int main(int argc, char** argv){
    ...
    ros::init(argc, argv, "disparity_camera_client_gpu");

    ros::NodeHandle nh;

    message_filters::Subscriber<sensor_msgs::Image> *img1_sub = new message_filters::Subscriber<sensor_msgs::Image>(nh,"/theia/left/image_rect",1);
    message_filters::Subscriber<sensor_msgs::Image> *img2_sub = new message_filters::Subscriber<sensor_msgs::Image>(nh,"/theia/right/image_rect",1);
    message_filters::sync_policies::ApproximateTime<Image, Image> MySyncPolicy(1000);
    MySyncPolicy.setAgePenalty(0.01);
    ...
    std::string serverName = "disparity_camera_server_gpu";
    Client ac_(serverName);
    clientPointer = &ac_;

    ROS_WARN("Waiting for '%s' server to start....",serverName.c_str());
    ac_.waitForServer();
